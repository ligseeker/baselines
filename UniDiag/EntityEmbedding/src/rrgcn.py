...
class RGCNCell(BaseRGCN):
    ...
    def forward(self, g, init_ent_emb, init_rel_emb):
        if self.encoder_name == "uvrgcn":
            ...
            # for multi layers:
            while len(r)<self.num_hidden_layers:
                r.append(init_rel_emb[-1])
...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self, g_list, static_graph, use_cuda):
        ...
        for i, g in enumerate(g_list):
            # x-input:pooling（related to relation）
            g = g.to(self.gpu)
            temp_e = self.h[g.r_to_e] # all entities related to this relation at current time.
            ...
            for span, r_idx in zip(g.r_len, g.uniq_r):
                x = temp_e[span[0]:span[1],:] # relavant entities embedding.
                x_mean = torch.mean(x, dim=0, keepdim=True)
                x_input[r_idx] = x_mean

            # GRU:
            # self.h_0：relation embedding from GRU
            if i == 0:
                x_input = torch.cat((self.emb_rel, x_input), dim=1)
                self.h_0 = self.relation_cell_1(x_input, self.emb_rel)    # first layer input
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0
            else:
                x_input = torch.cat((self.emb_rel, x_input), dim=1)
                self.h_0 = self.relation_cell_1(x_input, self.h_0)  # second input == first input in next timestamp
                self.h_0 = F.normalize(self.h_0) if self.layer_norm else self.h_0

            # GCN output：current_h:entity embedding（output from last layer of GCN）
            current_h = self.rgcn.forward(g, self.h, [self.h_0, self.h_0])

            current_h = F.normalize(current_h) if self.layer_norm else current_h
            # Time Gate：self.h：updating entity embedding
    ...
    def predict(self, test_graph, num_rels, static_graph, test_triplets, use_cuda):
        with torch.no_grad():
            ...
            inverse_test_triplets[:, 1] = inverse_test_triplets[:, 1] + num_rels  # inverse relation
            ...
            # modify：return entity and relation embedding
            return all_triples, score, score_rel,embedding,r_emb
    ...
    def get_loss(self, glist, triples, static_graph, use_cuda):
        ...
        # all_triples = triples # cuda out of memory
        all_triples = torch.cat([triples, inverse_triples])

        all_triples = all_triples.to(self.gpu)

        # return history_embs, static_emb, self.h_0, gate_list, degree_list
        # evolve_embs：embedding in history window  r_emb：relation embedding
        evolve_embs, static_emb, r_emb, _, _ = self.forward(glist, static_graph, use_cuda)
        # pre_emb：latest entity embedding
        ...
        if self.use_static:
            if self.discount == 1:
                ...
                for time_step, evolve_emb in enumerate(evolve_embs):
                    ...
                    mask = (math.cos(step) - sim_matrix) > 0
                    loss_static += self.weight * torch.sum(torch.masked_select(math.cos(step) - sim_matrix, mask))
        return loss_ent, loss_rel, loss_static,pre_emb,r_emb

    # modify: last timestamp embedding
    def get_loss_m(self, glist, static_graph, use_cuda):
        """
        model.get_loss(history_glist, output[0], static_graph, use_cuda)
        :param glist:
        :param triplets:
        ...
        return pre_emb,r_emb
    
    # modify for the last timestamp
    def predict_m(self, test_graph, static_graph, use_cuda):
        with torch.no_grad():
            evolve_embs, _, r_emb, _, _ = self.forward(test_graph, static_graph, use_cuda)
            embedding = F.normalize(evolve_embs[-1]) if self.layer_norm else evolve_embs[-1]

            return embedding,r_emb