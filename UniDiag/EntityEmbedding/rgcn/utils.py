...
def sort_and_rank_filter(batch_a, batch_r, score, target, all_ans):
    ...
    # indices = torch.nonzero(indices == target.view(-1, 1))  # indices : [B, 2] first column: increment. second column: matched entity id's location in every row
    indices = torch.nonzero(indices == target.view(-1, 1),as_tuple=False)  # indices : [B, 2] first column: increment. second column: matched entity id's location in every row
    indices = indices[:, 1].view(-1)
    return indices
...
def build_sub_graph(num_nodes, num_rels, triples, use_cuda, gpu):
    ...
    def comp_deg_norm(g):
        ...
        norm = 1.0 / in_deg
        return norm
    # os.system('echo {} >> {}'.format('[[['+str(len(triples)),'/home/zhangshenglin/zhaoyongxin/code/RE-GCN-master-wsr/src/out.txt'))
    src, rel, dst = triples.transpose()
    # os.system('echo {} >> {}'.format('{{{'+str(len(src)),'/home/zhangshenglin/zhaoyongxin/code/RE-GCN-master-wsr/src/out.txt'))
    # os.system('echo {} >> {}'.format('}}}'+str(len(dst)),'/home/zhangshenglin/zhaoyongxin/code/RE-GCN-master-wsr/src/out.txt'))
    # inverse
    ...
    # os.system('echo {} >> {}'.format('==='+str(len(g.nodes())),'/home/zhangshenglin/zhaoyongxin/code/RE-GCN-master-wsr/src/out.txt'))
    g.add_edges(src, dst)
    # os.system('echo {} >> {}'.format('==='+str(len(g.nodes())),'/home/zhangshenglin/zhaoyongxin/code/RE-GCN-master-wsr/src/out.txt'))
...
def split_by_time_old(data):
    ...
    for i in range(len(data)):
        t = data[i][3]
        train = data[i]
        # latest_t: last triple timestamp. It requires returned triples should follow time order.
        if latest_t != t:  # triples in the same timestamp
            # show snapshot
            latest_t = t
            if len(snapshot):
...
def slide_list(snapshots, k=1):
    ...
    :return:
    """
    k = k  # k=1 requires k length history and k+1 length label
    if k > len(snapshots):
...
def load_data_m(path,dataset, bfs_level=3, relabel=False):
    # path not exists
