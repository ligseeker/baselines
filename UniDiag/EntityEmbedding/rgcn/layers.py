...
class RGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, bias=None, activation=None,
        ...
        if self.skip_connect:
            self.skip_connect_weight = nn.Parameter(torch.Tensor(out_feat, out_feat)) # different from self-loop. it's skip connect
            nn.init.xavier_uniform_(self.skip_connect_weight,
                                    gain=nn.init.calculate_gain('relu'))

            self.skip_connect_bias = nn.Parameter(torch.Tensor(out_feat))
            nn.init.zeros_(self.skip_connect_bias)  # initialize zero
    ...
    def forward(self, g, prev_h=[]):
        ...
        if len(prev_h) != 0 and self.skip_connect:
            skip_weight = F.sigmoid(torch.mm(prev_h, self.skip_connect_weight) + self.skip_connect_bias) # sigmoid: limit value between 0 and 1
            # print("skip_ weight")
            # print(skip_weight)
        ...
        self.propagate(g)  # messages from neighbour entities.
        ...
        # print(len(prev_h))
        if len(prev_h) != 0 and self.skip_connect:  # Different ways to compute loop_message: activation and weighted.
...
class UnionRGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1,  bias=None,
        ...
        if self.skip_connect:
            ...
            self.skip_connect_bias = nn.Parameter(torch.Tensor(out_feat))
            nn.init.zeros_(self.skip_connect_bias)  # initialize as zero
    ...
    def msg_func(self, edges):
        ...
        # compute message from every node. Consider nodes who send messages as head entities.
